...
if not st.session_state.get('logged_in', False):
    ...
    st.info("Are you a Roaster, Barista, or a Home Brewer? Please log in to continue.")
...
if role == "Roaster":
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, = st.tabs([" ROASTER INTELLIGENCE ", " ROASTED BEANS ", " CUPPING SESSION ", " BREWING LOG ", " HISTORY ", " FLAVOR WHEEL "]) 
    ...
    with tab1:
        ...
        with st.form(key="roaster_form"):
            ...
            target_profile_notes = st.text_area("Target Profile Notes")
            ...
            if st.form_submit_button("LOG ROAST PROFILE"):
                roast_data = {
                    ...
                    "target_profile": target_profile_notes,
                    "remarks": remarks
                }
    ...
    tab1, tab2, tab3, = st.tabs([" BREWING LOG ", " HISTORY ", " FLAVOR WHEEL "])

    with tab1:
        st.subheader("Brewing Log")
        with st.form(key="brewing_form"):
            b_name = st.text_input("Bean Name")
            method = st.text_input("Brew Method (e.g., V60)")
            b_notes = st.text_area("Brewing Notes")
